...
void fixedListCastExecFunction<CastChildFunctionExecutor>(
    ...
    auto numOfEntry = reinterpret_cast<CastFunctionBindData*>(dataPtr)->numOfEntry;
...
static void resolveNestedVector(std::shared_ptr<ValueVector> inputVector, ValueVector* resultVector,
    uint64_t numOfEntry, CastFunctionBindData* dataPtr) {
    ...
    while (true) {
        if (inputType->getPhysicalType() == PhysicalTypeID::VAR_LIST &&
            ...
            memcpy(resultVector->getData(), inputVector->getData(),
                numOfEntry * resultVector->getNumBytesPerValue());
            resultVector->setNullFromBits(inputVector->getNullMaskData(), 0, 0, numOfEntry);

            numOfEntry = ListVector::getDataVectorSize(inputVector.get());
            ListVector::resizeDataVector(resultVector, numOfEntry);
            ...
            for (auto i = 0u; i < inputFieldVectors.size(); i++) {
                resolveNestedVector(
                    inputFieldVectors[i], resultFieldVectors[i].get(), numOfEntry, dataPtr);
            }
            return;
        } else {
            break;
...
static void varListCastExecFunction(
    ...
    auto numOfEntry = inputVector->state->selVector
                          ->selectedPositions[inputVector->state->selVector->selectedSize - 1] +
                      1;
    resolveNestedVector(
        inputVector, &result, numOfEntry, reinterpret_cast<CastFunctionBindData*>(dataPtr));
}
...
static std::unique_ptr<ScalarFunction> bindCastBetweenNested(
    ...
    case LogicalTypeID::STRUCT: {
        if (sourceTypeID == targetTypeID) {
            return std::make_unique<ScalarFunction>(functionName,
                std::vector<LogicalTypeID>{sourceTypeID}, targetTypeID, varListCastExecFunction);
        }
    }
    default:
        // lcov_excl_start
