...
std::unique_ptr<PhysicalOperator> PlanMapper::mapIndexScanNode(LogicalOperator* logicalOperator) {
    ...
    if (logicalIndexScan->getNumInfos() > 1) {
        ...
        for (auto i = 0u; i < logicalIndexScan->getNumInfos(); ++i) {
            ...
            auto storageIndex = nodesStore.getNodeTable(info->nodeTableID)->getPKIndex();
            ...
            indexLookupInfos.push_back(std::make_unique<IndexLookupInfo>(
                info->indexType->copy(), storageIndex, keyPos, offsetPos));
        }
        ...
        auto nodeTable = storageManager.getNodesStore().getNodeTable(info->nodeTableID);
