...
void NodeTableData::prepareCommit(LocalTable* localTable) {
    auto localNodeTable = ku_dynamic_cast<LocalTable*, LocalNodeTable*>(localTable);
    ...
    for (auto& [nodeGroupIdx, nodeGroup] : localNodeTable->nodeGroups) {
        for (auto columnID = 0; columnID < columns.size(); columnID++) {
            ...
            column->prepareCommitForChunk(nodeGroupIdx, nodeGroup->getLocalColumnChunk(columnID),
                nodeGroup->getInsertInfoRef(columnID), nodeGroup->getUpdateInfoRef(columnID),
                {} /* deleteInfo */);
        }
    }
}
