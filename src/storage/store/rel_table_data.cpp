...
void RelTableData::prepareCommitCSRColumns(LocalRelTableData* localTableData) {
    for (auto& [nodeGroupIdx, nodeGroup] : localTableData->nodeGroups) {
        ...
        if (relNodeGroupInfo->deleteInfo.empty() && relNodeGroupInfo->adjInsertInfo.empty()) {
            ...
            // on csr offsets.
            prepareCommitCSRNGWithoutSliding(nodeGroupIdx, relNodeGroupInfo, csrOffsetChunk.get(),
                relIDChunk.get(), nodeGroup.get());
        } else {
            ...
            prepareCommitCSRNGWithSliding(nodeGroupIdx, relNodeGroupInfo, csrOffsetChunk.get(),
                relIDChunk.get(), nodeGroup.get());
        }
    }
}
...
void RelTableData::prepareCommitCSRNGWithoutSliding(node_group_idx_t nodeGroupIdx,
    ...
    for (auto columnID = 0u; columnID < columns.size(); columnID++) {
        ...
        if (!csrOffsetToRowIdx.empty()) {
            ...
            columns[columnID]->prepareCommitForChunk(nodeGroupIdx, localChunk, {} /* insertInfo */,
                csrOffsetToRowIdx, {} /* deleteInfo */);
        }
    }
}
...
std::unique_ptr<ColumnChunk> RelTableData::slideCSROffsetColumnChunk(
    ...
    for (auto i = currentNumSrcNodesInNG; i < StorageConstants::NODE_GROUP_SIZE; i++) {
        ...
        if (relNodeGroupInfo->adjInsertInfo.contains(nodeOffset)) {
            ...
            numRowsInCSR += relNodeGroupInfo->adjInsertInfo.size();
...
std::unique_ptr<ColumnChunk> RelTableData::slideCSRColunnChunk(ColumnChunk* csrOffsetChunk,
...
void RelTableData::prepareCommitCSRNGWithSliding(node_group_idx_t nodeGroupIdx,
    CSRRelNGInfo* relNodeGroupInfo, ColumnChunk* csrOffsetChunk, ColumnChunk* relIDChunk,
    LocalRelNG* localNodeGroup) {
    ...
    auto slidedAdjColumnChunk = slideCSRColunnChunk(csrOffsetChunk, slidedCSROffsetChunk.get(),
        relIDChunk, relNodeGroupInfo->adjInsertInfo, {}, relNodeGroupInfo->deleteInfo,
        nodeGroupStartOffset, adjColumn.get(), localNodeGroup->getAdjColumn());
    adjColumn->append(slidedAdjColumnChunk.get(), nodeGroupIdx);
    slidedAdjColumnChunk.reset();
    for (auto columnID = 0u; columnID < columns.size(); columnID++) {
        auto slidedColumnChunk = slideCSRColunnChunk(csrOffsetChunk, slidedCSROffsetChunk.get(),
            relIDChunk, relNodeGroupInfo->insertInfoPerColumn[columnID],
            relNodeGroupInfo->updateInfoPerColumn[columnID], relNodeGroupInfo->deleteInfo,
