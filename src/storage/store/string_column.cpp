...
StringColumn::StringColumn(LogicalType dataType, const MetadataDAHInfo& metaDAHeaderInfo,
    ...
    dataColumn = std::make_unique<Column>(LogicalType(LogicalTypeID::UINT8),
        *metaDAHeaderInfo.childrenInfos[0], dataFH, metadataFH, bufferManager, wal, transaction,
        stats, false, false /*requireNullColumn*/);
    offsetColumn = std::make_unique<Column>(LogicalType(LogicalTypeID::UINT64),
...
bool StringColumn::canCommitInPlace(
    ...
    auto dataColumnMetadata = getDataColumn()->getMetadata(nodeGroupIdx, TransactionType::WRITE);
    ...
    auto indexColumnMetadata = getMetadata(nodeGroupIdx, TransactionType::WRITE);
    auto offsetColumnMetadata =
        getOffsetColumn()->getMetadata(nodeGroupIdx, TransactionType::WRITE);
    auto offsetCapacity =
    ...
    // Check if indices can be updated in-place
    if (totalStringsAfterUpdate > offsetCapacity ||
        ...
        !indexColumnMetadata.compMeta.canUpdateInPlace(
            (const uint8_t*)&totalStringsAfterUpdate, 0, getDataType().getPhysicalType())) {
        return false;
    }
    ...
    if (offsetColumnMetadata.numValues + newStrings >
        ...
        !offsetColumnMetadata.compMeta.canUpdateInPlace(
            (const uint8_t*)&totalStringDataAfterUpdate, 0, getDataType().getPhysicalType())) {
        return false;
    }
    return true;
}
